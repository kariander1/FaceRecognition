...
def plot_decision_boundary_2d(
    ...
    x2_max = max(ax.get_ylim())
    x1_axis = torch.linspace(x1_min, x1_max, int((x1_max - x1_min) / dx))
    x2_axis = torch.linspace(x2_min, x2_max, int((x2_max - x2_min) / dx))
    x1_grid, x2_grid = torch.meshgrid(x1_axis, x2_axis)
    x1_grid_flat = torch.flatten(x1_grid)
    x2_grid_flat = torch.flatten(x2_grid)

    x_data = torch.vstack((x1_grid_flat, x2_grid_flat)).T
...
def select_roc_thresh(
    ...
    y_score = y_pred[:, pos_label].detach()

    [fpr, tpr, thresh] = roc_curve(y_true=y, y_score=y_score,pos_label=pos_label)
    distance_array = (fpr - 0) ** 2 + (tpr - 1) ** 2

    optimal_thresh_idx = numpy.argmin(distance_array)
